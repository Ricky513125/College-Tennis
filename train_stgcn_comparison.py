#!/usr/bin/env python3
"""
Train STGCN++ model with skeleton data for comparison with MD-FED Stage 3.

This script trains a STGCN++ model using only skeleton (pose) data on the 
manual_annotations.json dataset. The goal is to compare STGCN++ (skeleton) 
with MD-FED Stage 3, VTN, I3D, and TSM models.

Usage:
    python train_stgcn_comparison.py \
        --manual_annotations manual_annotations.json \
        --frame_dir /path/to/frames \
        --pose_dir /path/to/skeletons \
        --save_dir ./stgcn_outputs \
        --clip_len 96 \
        --batch_size 4 \
        --num_epochs 500
"""
...
class STGCN_MD_FED(nn.Module):
    """
    使用 STGCN++ 作为 skeleton feature extractor 的模型
    只使用 skeleton (pose) 数据
    """
    def __init__(self, num_classes, clip_len, skeleton_arch='stgcn++', 
                 temporal_arch='gru'):
        super().__init__()
    ...
    def forward(self, frames=None, flow=None, skeleton=None):
        """
        Args:
            frames: Not used (only skeleton)
            flow: Not used (only skeleton)
        ...
        # Predictions
        coarse_pred = self._coarse_pred(sk_feat)  # [batch, clip_len, 2]
...
def prepare_stgcn_data(manual_annotations_file, output_dir, dataset_name='ncaa-rally-stgcn', train_ratio=0.8):
    """
    准备 STGCN 训练数据
    """
    print(f"Preparing STGCN training data...")
...
def train_stgcn(args):
    ...
    # Create model
    print("\nStep 2: Creating STGCN++ model...")
    print("✅ 使用 STGCN++ 作为 skeleton feature extractor")
    ...
    train_data = ActionSeqDataset(
        ...
        num_samples=-1, flow_dir=None, pose_dir=args.pose_dir,
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch_idx, batch in enumerate(train_pbar):
            ...
            skeleton = batch['skeleton'].cuda()  # STGCN uses skeleton data
...
def main():
    parser = argparse.ArgumentParser(
        description='Train STGCN++ model for comparison experiment'
    )
    parser.add_argument(
        '--manual_annotations',
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help='Path to extracted skeleton (pose) data'
    )
    parser.add_argument(
        '--save_dir',
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=224,
        help='Crop dimension for images (default: 224, not used for skeleton)'
    )
    parser.add_argument(
        '--batch_size',
        type=int,
