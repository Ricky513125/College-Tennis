"""
STGCN++ Stage 1: Pre-training on F3Set Dataset

This script pre-trains the STGCN++ model on the F3Set dataset using skeleton (pose) data,
...
Usage:
    python train_stgcn_stage1.py \
        ...
        --pose_dir /path/to/f3set_skeletons \
        --save_dir ./stgcn_outputs/stage1 \
        --clip_len 96 \
        --batch_size 4 \
        --num_epochs 500 \
        --skeleton_arch stgcn++ \
...
from train_stgcn_comparison import STGCN_MD_FED
...
def train_epoch(model, train_loader, optimizer, scaler, device, fg_weight=5.0):
    ...
    for batch in pbar:
        ...
        skeleton = batch['skeleton'].to(device)  # STGCN uses skeleton
...
def main():
    parser = argparse.ArgumentParser(description='STGCN++ Stage 1: Pre-training on F3Set')
    
    # Data paths
    parser.add_argument('--frame_dir', type=str, required=True,
                        help='Path to F3Set extracted frames')
    parser.add_argument('--pose_dir', type=str, required=True,
                        help='Path to F3Set extracted skeleton (pose) data')
    ...
    print("=" * 80)
    print("STGCN++ Stage 1: Pre-training on F3Set")
    print("=" * 80)
    print()
