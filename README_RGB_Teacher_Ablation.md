# RGB as Teacher 消融实验说明

## 📋 消融实验的目的

这个实验是为了测试**使用 RGB 作为教师网络**的效果，与原始 MD-FED（Skeleton 作为教师）进行对比。
...
### 步骤 2: 训练 Stage 2 (RGB as Teacher)
...
python train_stage2_rgb_teacher.py \
    ...
    --save_dir ./md_fed_outputs/stage2_rgb_teacher \
...
python few_shot_learning_stage3.py \
    --stage2_checkpoint_dir ./md_fed_outputs/stage2_rgb_teacher \
    --manual_annotations manual_annotations.json \
    --frame_dir /mnt/ssd2/lingyu/College-Tennis/ncaa_frames_rally \
    --flow_dir /mnt/ssd2/lingyu/College-Tennis/ncaa_optical_flow_rally \
    --save_dir ./md_fed_outputs/stage3_rgb_teacher \
    --num_epochs 500 \
    --batch_size 4 \
...
**分析**：
- 如果 RGB as Teacher 效果更好，说明视觉特征作为教师更有效
- 如果 MD-FED 效果更好，说明 Skeleton 作为教师的设计是正确的
...
1. **Stage 1 预训练是必需的**
   - 即使 RGB 作为教师，Skeleton 网络仍然需要 Stage 1 预训练
